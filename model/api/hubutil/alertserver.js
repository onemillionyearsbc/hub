...
logger.info('AlertMailServer Starting...');
...
async function doSearch(alertId, name) {
    ...
    try {
        ...
        let markup = createMarkup(adverts, results.length, name);
...
async function main() {
    ...
    logger.info("Will send out daily job results every day at " + zeroh + rule.hour + ":" + zero + rule.minute);
    ...
    bnUtil.connection.on('event', (event) => {
        ...
        switch (fqn) {
            case ALERT_EVENT:
                ...
                break;
            // TODO case JOB_APPLICATION_EVENT
            default:
...
function createMarkup(adverts, num, name) {
    let s = "s";
    if (num == 1) {
        s = "";
    }
